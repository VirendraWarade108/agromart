...
exports.getInvoicePDF = exports.getInvoiceJSON = exports.generateInvoice = void 0;
...
const generateInvoice = async (orderId, userId) => {
    ...
    if (userId && order.userId !== userId) {
        throw new errorHandler_1.AppError('Unauthorized to access this invoice', 403);
    }
    // Calculate subtotal
 ...
 * Get invoice as JSON
 */
const getInvoiceJSON = async (orderId, userId) => {
    return await (0, exports.generateInvoice)(orderId, userId);
};
 ...
 */
const getInvoicePDF = async (orderId, userId, res) => {
    const invoice = await (0, exports.generateInvoice)(orderId, userId);
    // Create PDF document
    ...
    doc
        ...
        .text(invoice.pricing.shippingCharges === 0 ? 'FREE' : `₹${invoice.pricing.shippingCharges.toFixed(2)}`, valuesX, yPosition, { align: 'right' });
    yPosition += 20;
    // Total line
    doc
        .strokeColor('#10b981')
    ...
    doc
        .fontSize(8)
        .fillColor('#9ca3af')
        .text('Thank you for your business! For support, contact support@agromart.com', 50, 750, { align: 'center', width: 495 });
    // Finalize PDF
    doc.end();
};
